    ...
    <div class="wrap">
        ...
        <div class="card">
            <div class="profile-row">
                <div class="avatar" id="avatarInitials"><?php echo htmlspecialchars(getInitials($thongTin['hoTen'] ?? '')); ?></div>
                <div class="profile-info">
                    <div class="name" id="displayName"><?php echo htmlspecialchars($thongTin['hoTen'] ?? ''); ?></div>
                    <div class="verified">
            ...
            <div class="form-grid">
                <div class="field">
                    ...
                    <input type="text" id="hoVaTen" value="<?php echo htmlspecialchars($thongTin['hoTen'] ?? ''); ?>" disabled>
                </div>
                <div class="field">
                    <label for="soDienThoai">Số điện thoại (Tên đăng nhập)</label>
                    <div class="input-lock-wrapper">
                        <input type="tel" id="soDienThoai" value="<?php echo htmlspecialchars($thongTin['soDienThoai'] ?? ''); ?>" readonly>
                        <i class="fa-solid fa-lock"></i>
                    </div>
                </div>
                <div class="field">
                    <label for="emailChinh">Email chính</label>
                    <input type="email" id="emailChinh" value="<?php echo htmlspecialchars($thongTin['email'] ?? ''); ?>" disabled>
                </div>
            </div>
            ...
            <div id="addressList">
                <?php if (!empty($diaChiList)): ?>
                    <?php foreach ($diaChiList as $dc): ?>
                        <div class="addr-item<?php echo $dc['laMacDinh'] ? ' is-default' : ''; ?>" data-id="<?php echo $dc['idDiaChi']; ?>">
                            <div class="addr-icon">
                            ...
                            <div class="addr-body">
                                <div class="addr-title-row">
                                    ...
                                    <?php if ($dc['laMacDinh']): ?><span class="badge badge-default">Mặc định</span><?php endif; ?>
                                </div>
...
<script>
        ...
        addrModalOverlay.addEventListener('click', (e) => { if (e.target === addrModalOverlay) closeAddressModal(); });
        ...
        document.getElementById('addressList').addEventListener('click', function (e) {
            ...
            fetch(`<?php echo URLROOT; ?>/khachHang/thongTinCaNhan/xoaDiaChi/${idDiaChi}`, { method: 'POST' })
                .then(res => res.json())
                .then(res => {
                    if (res.status) {
                        item.remove();
        ...
        document.getElementById('addressList').addEventListener('click', function (e) {
            ...
            fetch(`<?php echo URLROOT; ?>/khachHang/thongTinCaNhan/datMacDinh/${idDiaChi}`, { method: 'POST' })
                .then(res => res.json())
                .then(res => {
                    if (res.status) {
                        window.location.reload();
